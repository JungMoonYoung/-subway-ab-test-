...
> ※ 본 프로젝트는 실제 사용자 데이터를 사용하지 않으며, 현실적인 가정을 기반으로 한 **합성 데이터 시뮬레이션 실험**입니다.
...
- 반복 측정 통계 분석 (GEE 중심, Mixed-Effects Model, FDR 보정)
- 시각화 및 자동 리포트 생성
- (선택) Streamlit 대시보드
...
**프로젝트 완료**: 2025년 9월 ~ 11월 (순수 개발 약 5주, 학업 병행으로 일부 중단)
...
- 서울 지하철 일일 이용객 800만 명 기준, **약 45만 명의 경로 선택 유도 가능**
  - → UI 설계 변경만으로도 추가 비용 없이 혼잡도 분산 효과를 기대할 수 있음
- 통계적으로 매우 유의미 (p<0.001, 우연 확률 0.1% 미만)
- ※ UI 변경처럼 비용이 거의 들지 않는 실험에서는 Small effect라도 서비스 전체에는 큰 영향을 줄 수 있음

### 2. 사용자 행동 예측 요인 (GEE 모델)

**반복 측정 데이터의 상관관계를 고려한 GEE(Generalized Estimating Equations) 모델**로 사용자 행동의 주요 결정 요인을 분석했습니다.

<details>
<summary><b>📊 로지스틱 회귀 계수 상세 분석 (클릭하여 펼치기)</b></summary>

| 변수 | 계수 (β) | 오즈비 (OR) | 해석 | p-value |
|------|---------|------------|------|---------|
| **UI 그룹** (A vs B) | +0.330 | 1.39 | A그룹의 Fast 선택 오즈가 B그룹 대비 1.39배 (39% 증가) | <0.001*** |
| **시간 압박** | +0.936 | 2.55 | 압박 1단계 증가 시 Fast 선택 오즈가 2.55배 (155% 증가) | <0.001*** |
| **효율 지향 성향** | +0.595 | 1.81 | 효율지향 성향 1단계당 Fast 선택 오즈가 1.81배 (81% 증가) | <0.001*** |
| **Trial 증가** (학습 효과) | -0.404 | 0.67 | Trial 1회 증가 시 Fast 선택 오즈가 0.67배 (33% 감소) | <0.001*** |
| **시간 차이** | +0.132 | 1.14 | 시간 차이 1분당 Fast 선택 오즈가 1.14배 (14% 증가) | <0.001*** |
| **혼잡도 차이** | -0.009 | 0.991 | 혼잡도 차이 1%p당 Fast 선택 오즈가 0.991배 (0.9% 감소) | <0.001*** |

***: p < 0.001 (통계적으로 매우 유의미)
**오즈비(OR)** = exp(계수): 로지스틱 회귀에서 독립변수 1단위 변화 시 종속변수 선택 오즈의 비율
...
- ✅ **대규모 반복 측정 데이터**: 100,000명 × 5회 반복 실험 설계 및 분석
